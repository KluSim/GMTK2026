...
class Customer:
	...
	var wantsstring: String
	func _init(gela: float,  picky: float, desires: DesireProps, ws: String) -> void:
		gelassenheit = gela
		desireProps = desires
		pickyness = picky
		wantsstring = ws
...
class Funcs:
	static func offer(stone: StoneProps, customer: Customer, reputation: float):
		...
		return prize
	static func stoneprop_dist(desire: DesireProps, stone: StoneProps) -> Dist:
		var dist = 0.0
		var resason = "shape"
...
#Array[Array[Customer]], day, nr customer, customer
	...
	static func get_stone_color(stonecolor: StoneColor) -> Color:
